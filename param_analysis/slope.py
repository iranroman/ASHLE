...
b_1b = -1
f_0 = 1
f_stim = [1, 1.71930111, 1.33295354, 0.76859625, 0.55374987]
l1s = [0.3, 0.4, 0.5, 0.6]
l2s = [0.0, 0.024, 0.05] 
base= np.exp(1)
nlearn = 0
    ...
    for il2, l2 in enumerate(l2s):
        ...
        for if_s, f_s in enumerate(f_stim):
            ...
            for n, t in enumerate(time[:-1]):
                ...
                f_m[n+1] = f_m[n] + T*f_m[n]*(-l1*np.real(x[n])*np.sin(np.angle(z_m[n])) - (l2/4)*(np.power(base,f_m[n]) - np.power(base,f_b[n]))/np.power(base,f_b[n]))
            ...
            if f_s == f_0:
                ...
                bar_results[if_s - 1] = slope - mean_slop - mean_slope
